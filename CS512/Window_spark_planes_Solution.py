### gs://cs-512/Window_spark_planes_Solution.py
...
def haversine(lon1, lat1, lon2, lat2):
    ...
    distance = float(c * r)
    if distance > 29: # only included code that runs the data I "decided was best"
        return None
    else:
        return distance

def To_numb(x):
  # I don't have FSeen as it appeared in all the other examples, FSeen == PosTime, so I made a blank column in dataprep. As such I copy it here.
  x['PosTime'] = int(x['FSeen']) 
  x['FSeen'] = int(x['FSeen'])
  x['Lat'] = float(x['Lat'])
  x['Long'] = float(x['Long'])
  return x
...
conf={
    ...
    'mapred.bq.input.project.id': "gh-bigq-bigd",
    'mapred.bq.input.dataset.id': 'yelp_reviews',
    'mapred.bq.input.table.id': 'yelp_reviews',
}
...
## Add HAVING SUM(dist) < {some number} to get the chatgpt and google answers by only filtering out total flight distance less than a "record"
df1.createOrReplaceTempView('planes')
top = spark.sql("Select Icao, SUM(dist)\
                AS dist FROM planes\
                GROUP BY Icao\
                ORDER BY dist desc\
                LIMIT 10")
...
max = spark.sql("Select MAX(dist) FROM planes")
avg = spark.sql("Select AVG(dist) FROM Planes")
median, q1, q3 = df1.approxQuantile('dist', [.5, .25, .75], 0) # get summary
pprint.pprint(miles.collect())
pprint.pprint(max.collect())
pprint.pprint(avg.collect())
# print summaries
